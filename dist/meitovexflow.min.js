(function(D,l,u){function ra(){y.DEBUG&&Vex.L("Vex.Flow.Syllable",arguments)}Vex.Flow.Annotation.prototype.setMeiElement=function(a){this.meiElement=a;return this};Vex.Flow.Annotation.prototype.getMeiElement=function(){return this.meiElement};Vex.Flow.Annotation.format=function(a,b){if(!a||0===a.length)return!1;for(var c=b.text_line,d=b.top_text_line,f=b.bottom_text_line,e,g=0;g<a.length;++g){var h=a[g];e=h.font.size/10*1.5;1===h.vert_justification?(h.setTextLine(d),d+=e):3===h.vert_justification?
...
(this.render_options.max_slope-this.render_options.min_slope)/this.render_options.slope_iterations,g=Number.MAX_VALUE,h=0,k=0,n=this.render_options.min_slope;n<=this.render_options.max_slope;n+=c){for(var m=0,p=0,q=1;q<this.notes.length;++q){var A=this.notes[q],l=A.getStemX(),v=A.getStemExtents().topY;A.isRest()&&(v=-1===this.stem_direction?v+70:v+10);A=this.getSlopeY(l,a,b,n)+p;v*this.stem_direction<A*this.stem_direction?(v=Math.abs(v-A),p+=v*-this.stem_direction,m+=v*q):m+=(v-A)*this.stem_direction}q=
this.notes[this.notes.length-1];q=(q.getStemExtents().topY-b)/(q.getStemX()-a)/2;q=Math.abs(q-n);m=this.render_options.slope_cost*q+Math.abs(m);m<g&&(g=m,h=n,k=p)}this.slope=h;this.y_shift=k},applyStemExtensions:function(){for(var a=this.notes[0],b=a.getStemExtents().topY,a=a.getStemX(),e=0;e<this.notes.length;++e){var g=this.notes[e],h=g.getStemX(),k=g.getStemExtents(),n=k.baseY,k=k.topY,n=n+this.stem_direction*g.glyph.stem_offset,m=Vex.Flow.STEM_WIDTH;if(g.hasStem()){var p=this.getSlopeY(h,a,b,
this.slope)+this.y_shift,q=g.getStemDirection(),A=this.render_options.beam_width,l=A-1,v=0;if(q===this.stem_direction)p=Math.abs(k-p)-c.HEIGHT-1;else{for(var K,r=e;r--;)if(K=this.notes[r],K.stem_direction===this.stem_direction){v=K.getBeamCount();break}1<v&&(v=Math.min(v,g.getGlyph().beam_count),l+=(v-1)*A*1.5);p=-Math.abs(k-p)-c.HEIGHT+.5+l}g.setStem(new Vex.Flow.Stem({x_begin:h-Vex.Flow.STEM_WIDTH/2,x_end:h,y_top:1===q?k:n,y_bottom:1===q?n:k,y_extend:m,stem_extension:p,stem_direction:q}))}}},getBeamLines:function(a){for(var b,
c,g=[],h=!1,k,n=this.render_options.partial_beam_length,m=0;m<this.notes.length;++m){k=this.notes[m];b=this.notes[m-1];var p=this.notes[m+1],q=k.getIntrinsicTicks();c=p;p=0;c&&b&&(p=b.getBeamCount()-c.getBeamCount());c="8"!==a&&0>p;p=k.isRest()?k.getCenterGlyphX():k.getStemX();q<Vex.Flow.durationToTicks(a)?h?(k=g[g.length-1],k.end=p,k=-1!==this.break_on_indices.indexOf(m),q=8<=parseInt(a,10),k&&q&&(h=!1)):k.isRest()||(h={start:p,end:null,flipped:k.getStemDirection()!==this.stem_direction},b&&!c&&
(h.end=p-n),g.push(h),h=!0):(h&&(k=g[g.length-1],null==k.end&&(k.end=k.start+n)),h=!1)}!0===h&&(k=g[g.length-1],null==k.end&&(k.end=k.start-n));return g},drawStems:function(){this.notes.forEach(function(a){a.getStem()&&a.getStem().setContext(this.context).draw()},this)},drawBeamLines:function(){if(!this.context)throw new Vex.RERR("NoCanvasContext","Can't draw without a canvas context.");var a=["4","8","16","32","64"],b=this.notes[0],e=this.notes[this.notes.length-1],g=b.getStemExtents().topY;e.getStemExtents();
for(var h=b.getStemX(),k=this.render_options.beam_width*this.stem_direction,n=0;n<a.length;++n){for(var m=a[n],m=this.getBeamLines(m),p=0;p<m.length;++p){var q=m[p];if(!q.flipped){var A=q.start-(this.stem_direction==c.DOWN?Vex.Flow.STEM_WIDTH/2:0),l=this.getSlopeY(A,h,g,this.slope),q=q.end+(1==this.stem_direction?Vex.Flow.STEM_WIDTH/3:-Vex.Flow.STEM_WIDTH/3),v=this.getSlopeY(q,h,g,this.slope);this.context.beginPath();this.context.moveTo(A,l+this.y_shift);this.context.lineTo(A,l+k+this.y_shift);this.context.lineTo(q+
1,v+k+this.y_shift);this.context.lineTo(q+1,v+this.y_shift);this.context.closePath();this.context.fill()}}g+=1.5*k}k=this.render_options.beam_width*this.stem_direction*-1;g=b.getStemExtents().topY+.5*k;e.getStemExtents();h=b.getStemX();b=!1;for(n=0;n<a.length;++n){m=a[n];m=this.getBeamLines(m);for(p=0;p<m.length;++p)q=m[p],q.flipped&&(b=!0,A=q.start-(-1*this.stem_direction==c.DOWN?Vex.Flow.STEM_WIDTH/2:0),l=this.getSlopeY(A,h,g,this.slope),q=q.end+(1==-1*this.stem_direction?Vex.Flow.STEM_WIDTH/3:
-Vex.Flow.STEM_WIDTH/3),v=this.getSlopeY(q,h,g,this.slope),this.context.beginPath(),this.context.moveTo(A,l+this.y_shift),this.context.lineTo(A,l+k+this.y_shift),this.context.lineTo(q+1,v+k+this.y_shift),this.context.lineTo(q+1,v+this.y_shift),this.context.closePath(),this.context.fill());b&&(g+=1.5*k)}},preFormat:function(){return this},postFormat:function(){this.postFormatted||(this.calculateSlope(),this.applyStemExtensions(),this.postFormatted=!0)},draw:function(){if(!this.context)throw new Vex.RERR("NoCanvasContext",
...
a.direction,k=0;k<this.first_indices.length;++k){var n=(a.last_x_px+g+(a.first_x_px+e))/2,m=a.first_ys[this.first_indices[k]]+h,p=a.last_ys[this.last_indices[k]]+h;if(isNaN(m)||isNaN(p))throw new Vex.RERR("BadArguments","Bad indices for tie rendering.");var q=(m+p)/2+d*a.direction,l=(m+p)/2+f*a.direction;c.beginPath();c.moveTo(a.first_x_px+e,m);c.quadraticCurveTo(n,q,a.last_x_px+g,p);c.quadraticCurveTo(n,l,a.first_x_px+e,m);c.closePath();c.fill()}},renderText:function(a,c){if(this.text){var d;d=(a+
...
a.length;++g){h=a[g];k=h.getNote();var n;k instanceof Vex.Flow.StaveNote?(n=k.getKeyProps()[h.getIndex()],k=n.displaced?k.getExtraLeftPx():0,e.push({line:n.line,shift:k,str:h})):(n=k.getPositions()[h.getIndex()],e.push({line:n.str,shift:0,str:h}))}for(g=n=0;g<e.length;++g)h=e[g].str,k=e[g].shift,h.setXShift(f+k),n=Math.max(h.getWidth()+0,n);b.left_shift+=n;return!0};Vex.Inherit(a,b,{init:function(c,d){a.superclass.init.call(this);this.note=null;this.options=Vex.Merge({},d);this.all_voices="all_voices"in
...
"Can't draw stroke without a note and index.");var b=this.note.getModifierStartXY(this.position,this.index),d=this.note.getYs(),f=b.y,e=b.y,b=b.x-5,g=this.note.stave.options.spacing_between_lines_px,h=this.getModifierContext().getModifiers(this.note.getCategory()),k;for(k=0;k<h.length;k++)for(var d=h[k].getYs(),n=0;n<d.length;n++)if(this.note==h[k]||this.all_voices)f=Vex.Min(f,d[n]),e=Vex.Max(e,d[n]);var m,p,q,l,B;switch(this.type){case a.Type.BRUSH_DOWN:m="vc3";p=-3;q=f-g/2+10;e+=g/2;break;case a.Type.BRUSH_UP:m=
"v11";p=.5;q=e+g/2;f-=g/2;break;case a.Type.ROLL:f+=g/2;e=this.note instanceof Vex.Flow.StaveNote&&0!==(e-f)%2?e+.5*g:e+g;break;case a.Type.ROLL_DOWN:case a.Type.RASQUEDO_DOWN:m="vc3";p=-3;l=this.x_shift+p-2;this.note instanceof Vex.Flow.StaveNote?(f+=1.5*g,e=0!==(e-f)%2?e+.5*g:e+g,q=f-g,B=e+g+2):(f+=1.5*g,e+=g,q=f-.75*g,B=e+.25*g);break;case a.Type.ROLL_UP:case a.Type.RASQUEDO_UP:m="v52",p=-4,l=this.x_shift+p-1,this.note instanceof Vex.Flow.StaveNote?(f+=.5*g,0===(e-f)%2&&(e+=g/2),q=e+.5*g,B=f-1.25*
g):(f+=.25*g,e+=.5*g,q=e+.25*g,B=f-g)}if(this.type==a.Type.BRUSH_DOWN||this.type==a.Type.BRUSH_UP)this.context.fillRect(b+this.x_shift,f,1,e-f);else if(this.note instanceof Vex.Flow.StaveNote)for(k=f;k<=e;k+=g)Vex.Flow.renderGlyph(this.context,b+this.x_shift-4,k,this.render_options.font_scale,"va3");else{for(k=f;k<=e;k+=10)Vex.Flow.renderGlyph(this.context,b+this.x_shift-4,k,this.render_options.font_scale,"va3");this.type==Vex.Flow.Stroke.Type.RASQUEDO_DOWN&&(B=k+.25*g)}this.type!==a.Type.ROLL&&Vex.Flow.renderGlyph(this.context,
b+this.x_shift+p,q,this.render_options.font_scale,m);if(this.type==a.Type.RASQUEDO_DOWN||this.type==a.Type.RASQUEDO_UP)this.context.save(),this.context.setFont(this.font.family,this.font.size,this.font.weight),this.context.fillText("R",b+l,B),this.context.restore()}});return a}();Vex.Flow.Tremolo.prototype.draw=function(){if(!this.context)throw new Vex.RERR("NoContext","Can't draw Tremolo without a context.");if(!this.note||null==this.index)throw new Vex.RERR("NoAttachedNote","Can't draw Tremolo without a note and index.");
...
e=a.getElementsByTagName("staff");c=0;for(f=e.length;c<f;c++)g=e[c],h=Number(g.getAttribute("n")),-1===d.staves.indexOf(h)&&(g.parentNode.removeChild(g),c--,f--)}"measure"===a.localName&&Number(a.getAttribute("n"))===d.end_n&&(b=!1)}return{inside_slice:b,removed:k}},g=b.staves;if(g){var h="",k="";c=0;for(d=g.length;c<d;c++)h+=':not([n="'+g[c]+'"])',k+=':not([staff="'+g[c]+'"])'}var n=a.cloneNode(!0);if(g)for(g=n.querySelectorAll("staffDef"+h),c=0,d=g.length;c<d;c++)g[c].parentNode.removeChild(g[c]);
if(b.noClef||b.noKey||b.noMeter)c=n.getElementsByTagName("scoreDef")[0],d=c.getElementsByTagName("staffDef"),f([c],b),f(d,b);if(b.noConnectors)for(f=n.getElementsByTagName("staffGrp"),c=0,d=f.length;c<d;c++)f[c].removeAttribute("symbol");f=!1;c=n.getElementsByTagName("section")[0].childNodes;for(var m,p,q,g=0,h=c.length;g<h;g++)if(d=c[g],"ending"===d.localName){var l=d.childNodes;m=0;for(p=l.length;m<p;m++)q=e(l[m],f,k,b),f=q.inside_slice,q.removed&&(m--,p--);0===d.getElementsByTagName("measure").length&&
(d.parentNode.removeChild(d),g--,h--)}else q=e(d,f,k,b),f=q.inside_slice,q.removed&&(g--,h--);return n};window.MeiLib||(window.MeiLib={});MeiLib.Alt=function(a,b,c,d){this.elem=a;this.xmlID=b;this.altitems=[];this.parentID=c;this.tagname=d};MeiLib.Alt.prototype.getDefaultItem=function(){var a=function(a,c,d){var f,e;for(e in a){if(a[e].tagname===c)return a[e];f||a[e].tagname!==d||(f=a[e])}return f};if("choice"===this.tagname)return a(this.altitems,"corr","sic");if("app"===this.tagname)return a(this.altitems,
...
MeiLib.XMLID(c),b.alt_item=c):(a=a.getElementsByTagName("rdg")[0])?(b.alt_item_xml_id=MeiLib.XMLID(a),b.alt_item=a):b={};return b};MeiLib.MeiDoc.prototype.initSectionView=function(a){a=a||{};this.sectionview_score=this.rich_score.cloneNode(!0);this.sectionplane={};var b=this.sectionview_score.querySelectorAll("app, choice"),c,d=this.sectionview_score,f=this.sectionplane,e=this.ALTs,g=this.xmlDoc,h,k,n;k=0;for(n=b.length;k<n;k++){h=b[k];var m=MeiLib.XMLID(h),p=a[m];if(p){c=p.xmlID;var q=D(d).find(p.tagname+
'[xml\\:id="'+c+'"]')[0];if(!q)throw new MeiLib.RuntimeError("MeiLib.MeiDoc.prototype.initSectionView():E01","Cannot find <lem>, <rdg>, <sic>, or <corr> with @xml:id '"+c+"'.");}else if(p=this.ALTs[m].getDefaultItem())c=p.xmlID,q=p.elem;var p=h.parentNode,l=g.createProcessingInstruction("MEI2VF",'rdgStart="'+m+'"');p.insertBefore(l,h);if(q){var l=q.childNodes,r;for(r=0;r<l.length;++r)p.insertBefore(l.item(r).cloneNode(!0),h)}l=g.createProcessingInstruction("MEI2VF",'rdgEnd="'+m+'"');p.insertBefore(l,
h);p.removeChild(h);f[m]=[];e[m].altitems[c]&&f[m].push(e[m].altitems[c])}return this.sectionview_score};MeiLib.MeiDoc.prototype.updateSectionView=function(a){var b,c,d=function(a,b){var c=0,d,f,e;for(e in a){f=a[e];var g=0,h=void 0;for(h in f)f[h]!==u&&f[h]===b[h]&&g++;f=g;c<f&&(c=f,d=a[e])}return d};for(b in a){var f=this.ALTs,e=[];"string"===typeof a[b]&&(a[b]=[a[b]]);var g;c=a[b].length;if(0<c)for(g=0;g<c;g++)e.push(f[b].altitems[a[b][g]]);else(g=this.ALTs[b].getDefaultItem())&&e.push(g);var h=
this.altgroups[b];if(h)for(g=0;g<h.length;g++){c=h[g];var k=[],n,m;n=0;for(m=e.length;n<m;n++)k.push(d(f[c].altitems,e[n]));this.replaceAltInstance({appXmlID:c,replaceWith:k})}else this.replaceAltInstance({appXmlID:b,replaceWith:e})}};MeiLib.MeiDoc.prototype.replaceAltInstance=function(a){var b=function(a,b){var c;for(c=0;c<b.length;++c)a.push(b.item(c));return a},c=a.appXmlID,d=D(this.sectionview_score).find("[xml\\:id="+this.ALTs[c].parentID+"]")[0];if("undefined"!==typeof d){var f=d.childNodes,
e=a.replaceWith,g=[],h=this.rich_score;if(e){var k;for(k=0;k<e.length;++k)var n=e[k],m=n.xmlID,n=D(h).find(n.tagname+'[xml\\:id="'+m+'"]')[0],g=b(g,n.childNodes)}var e=function(a,b){a=a.replace("'",'"');b=b.replace("'",'"');return a===b},n=h=!1,p=null;k=0;for(b=f.length;k<b;k++)m=f[k],7===m.nodeType?"MEI2VF"===m.nodeName&&e(m.nodeValue,'rdgStart="'+c+'"')?n=h=!0:"MEI2VF"===m.nodeName&&e(m.nodeValue,'rdgEnd="'+c+'"')&&(h=!1,p=m):h&&(d.removeChild(m),k--,b--);if(!n)throw"processing instruction not found";
if(h)throw"Unmatched <?MEI2VF rdgStart?>";f=p?function(a){d.insertBefore(a,p)}:function(a){d.appendChild(a)};k=0;for(b=g.length;k<b;k++)f(g[k].cloneNode(!0));this.sectionplane[c]=a.replaceWith;return this.sectionview_score}};MeiLib.MeiDoc.prototype.getSectionViewSlice=function(a){return MeiLib.SliceMEI(this.sectionview_score,a)};MeiLib.MeiDoc.prototype.getRichSlice=function(a){var b=new MeiLib.MeiDoc;b.xmlDoc=this.xmlDoc;b.rich_head=this.rich_head.cloneNode(!0);b.rich_music=this.rich_music.cloneNode(!0);
...
h){e=0;b=b.childNodes;g=!1;for(h=0;h<b.length&&!g;++h)1===b[h].nodeType&&(g=d(b[h]),e+=g.beats,g=g.found);return{beats:e,found:g}}if("chord"===h){b.getAttribute("dur");if(b.getAttribute("xml:id")===a)return{beats:0,found:!0};if(h=b.getAttribute("dur"))return D(b).find("[xml\\:id='"+a+"']").length?{beats:0,found:!0}:{beats:MeiLib.dur2beats(h,c),found:g};b=b.childNodes;g=!1;for(h=0;h<b.length&&!g;++h)1===b[h].nodeType&&(g=d(b[h]),e=g.beats,g=g.found);return{beats:e,found:g}}return{beats:0,found:!1}};
return d(b)};var x=function(a){this.name="MEI2VF Runtime Error";this.message=a;this.stack=Error().stack};x.prototype=Error();var G=function(){},w={error:G,info:G,warn:G,debug:G,appender:{error:function(){window.console.error("MEI2VF ("+arguments[0]+"): "+Array.prototype.slice.call(arguments,1).join(" "))},info:function(){window.console.info("MEI2VF ("+arguments[0]+"): "+Array.prototype.slice.call(arguments,1).join(" "))},warn:function(){window.console.warn("MEI2VF ("+arguments[0]+"): "+Array.prototype.slice.call(arguments,
1).join(" "))},debug:function(){window.console.log("MEI2VF ("+arguments[0]+"): "+Array.prototype.slice.call(arguments,1).join(" "))}},setAppender:function(a){if("object"===typeof a){if("function"===typeof a.error&&"function"===typeof a.warn&&"function"===typeof a.info&&"function"===typeof a.debug)return this.appender=a,this;throw new x("Parameter object does not contain the expected appender methods.");}throw new x("Parameter is not an object");},setLevel:function(a){var b,c,d,f=!1;d=["debug","info",
...
r.serializeElement(a));}if(!h)throw new MeiLib.RuntimeError("MeiLib.durationOf:E06",'@dur of chord must be specified either in <chord> or in at least one of its <note> elements. Proceeding with default @dur="4". Element:'+r.serializeElement(a));return k*MeiLib.dur2beats(Number(h),b)},h=function(a,b){var c=0,l,r,t,v;t=a.childNodes;l=0;for(r=t.length;l<r;l++){v=t[l];var w=v.localName;v=d(v,w)?0:f(w)?e(v,b):"chord"===w?g(v,b):"beam"===w?h(v,b):"tuplet"===w?k(v,b):0;c+=v}return c},k=function(a,b){var c=
...
a?{beats:0,found:!0}:{beats:c.count,found:!1};if("layer"===h||"beam"===h||"tuplet"===h){e=0;b=b.childNodes;g=!1;for(h=0;h<b.length&&!g;++h)1===b[h].nodeType&&(g=d(b[h]),e+=g.beats,g=g.found);return{beats:e,found:g}}if("chord"===h){b.getAttribute("dur");if(b.getAttribute("xml:id")===a)return{beats:0,found:!0};if(h=b.getAttribute("dur"))return D(b).find("[xml\\:id='"+a+"']").length?{beats:0,found:!0}:{beats:MeiLib.dur2beats(h,c),found:g};b=b.childNodes;g=!1;for(h=0;h<b.length&&!g;++h)1===b[h].nodeType&&
(g=d(b[h]),e=g.beats,g=g.found);return{beats:e,found:g}}return{beats:0,found:!1}};return d(b)};var T=function(a){this.xmlid=a};T.prototype={setId:function(a){this.xmlid=a},setTStamp:function(a){this.tstamp=a;this.xmlid&&this.tryResolveReference(!0)},tryResolveReference:function(){if(!this.tstamp)throw new x("tstamp must be set in order to resolve reference.");this.xmlid=this.meicontext?MeiLib.tstamp2id(this.tstamp,this.meicontext.layer,this.meicontext.meter):null},getId:function(a){a&&a.meicontext&&
...
[];this.systemInfo=a;this.unresolvedTStamp2=b},validateAtts:function(){throw new x("You have to provide a validateAtts() method when inheriting MEI2VF.EventLinkCollection.");},createVexFromInfos:function(){throw new x("You have to provide a createVexFromInfos method when inheriting MEI2VF.EventLinkCollection.");},createInfos:function(a,b,c,d){var f=function(a){return{stave_n:a.getAttribute("staff")||"1",layer_n:a.getAttribute("layer")||"1"}},e=function(a,b,c){var e=f(b),g=c.querySelector('staff[n="'+
e.stave_n+'"]');if(!g)throw new x('Cannot find staff @n="'+e.stave_n+'" in '+r.serializeElement(c));b=g.querySelector('layer[n="'+e.layer_n+'"]');if(!b&&((g=g.getElementsByTagName("layer")[0])&&!g.hasAttribute("n")&&(b=g),!b))throw new x('Cannot find layer @n="'+e.layer_n+'" in '+r.serializeElement(c));c=d.getStaveInfo(e.stave_n);if(!c)throw new x("Cannot determine staff definition.");c=c.getTimeSpec();if(!c.count||!c.unit)throw new x("Cannot determine meter; missing or incorrect @meter.count or @meter.unit.");
return MeiLib.tstamp2id(a,b,c)},g,h,k,n,m,l;g=0;for(h=a.length;g<h;g++){n=a[g];k=new F(null,null);m=r.attsToObj(n);this.validateAtts(m);k.setParams(m);k.setMeiElement(n);if(l=m.startid)k.setFirstId(l.substring(1));else if(l=m.tstamp)l=e(l,n,b),k.setFirstId(l);if(l=m.endid)k.setLastId(l.substring(1));else if(l=m.tstamp2)m=+l.substring(0,l.indexOf("m")),0<m?(k.setLastTStamp(l.substring(l.indexOf("+")+1)),n=f(n),n=c+m+":"+n.stave_n+":"+n.layer_n,this.unresolvedTStamp2[n]||(this.unresolvedTStamp2[n]=
[]),this.unresolvedTStamp2[n].push(k)):(l=e(l.substring(l.indexOf("+")+1),n,b),k.setLastId(l));this.addModel(k)}},addModel:function(a){this.allModels.push(a)},getModels:function(){return this.allModels},setContext:function(a){this.ctx=a;return this},draw:function(){var a=this.ctx,b,c,d=this.allVexObjects;b=0;for(c=d.length;b<c;b++)d[b].setContext(a).draw()}};var U=function(a,b){this.init(a,b)};Vex.Inherit(U,z,{init:function(a,b){U.superclass.init.call(this,a,b)},validateAtts:function(a){if(!a.form)throw new x("@form is mandatory in <hairpin> - make sure the xml is valid.");
...
0===+c.index?"below":+c.index===g-1?"above":u)));b.system!==u&&c.system!==u&&b.system!==c.system?(this.createSingleTie(b,{},e.params),e.params.curvedir||(e.params.curvedir=-1===b.vexNote.getStemDirection()?"above":"below"),this.createSingleTie({},c,e.params)):this.createSingleTie(b,c,e.params)}return this},createSingleTie:function(a,b,c){b=new l.StaveTie({first_note:a.vexNote,last_note:b.vexNote,first_indices:a.index||(a.vexNote?[0]:b.index),last_indices:b.index||(b.vexNote?[0]:a.index)});c.curvedir&&
b.setDir("above"===c.curvedir?-1:1);a.vexNote&&!0===a.vexNote.grace&&(b.render_options.first_x_shift=-5);this.allVexObjects.push(b)}});var W=function(a,b){this.init(a,b)};Vex.Inherit(W,z,{init:function(a,b){W.superclass.init.call(this,a,b)},validateAtts:function(){},startSlur:function(a,b){var c=new F(a,null);c.setParams({linkCond:b});this.allModels.push(c)},terminateSlur:function(a,b){var c,d,f,e,g=this.getModels();c=function(a,b){return a.nesting_level===b.nesting_level};d=!1;for(f=0;f<g.length;++f)if((e=
g[f])&&!e.getLastId()&&c(e.params.linkCond,b)){e.setLastId(a);d=!0;break}d||this.addModel(new F(null,a))},createVexFromInfos:function(a){var b,c,d,f,e,g,h,k,n,m={};d=0;for(f=this.allModels.length;d<f;d++){e=this.allModels[d];h=e.params;k="above"===h.curvedir?1:"below"===h.curvedir?-1:null;b=a[e.getFirstId()]||{};c=a[e.getLastId()]||{};if(!b.vexNote&&!c.vexNote){var p;a="";for(p in h)a+=p+'="'+h[p]+'" ';console.log(e);w.warn("Slur could not be processed","No slur start or slur end could be found. Slur parameters: "+
a+". Skipping slur.");return!0}var q,r;b.vexNote&&(q=b.vexNote.getStemDirection());c.vexNote&&(r=c.vexNote.getStemDirection());n=b.layerDir||c.layerDir;e=q||r;(g=h.bezier)?(m.cps=this.bezierStringToCps(g),m.custom_cps=!0,k=0>m.cps[0].y?1:-1):(k||(k=n?n:-1*e),m.invert=!(-1===k&&1===r||1===k&&-1===r));g=parseFloat(h.startvo);h=parseFloat(h.endvo);m.y_shift_start=g;m.y_shift_end=h;b.vexNote&&c.vexNote&&b.vexNote.hasStem()&&c.vexNote.hasStem()?q!==r&&q&&r?e===k?(m.position=l.Curve.Position.NEAR_TOP,m.position_end=
l.Curve.Position.NEAR_HEAD):(m.position=l.Curve.Position.NEAR_HEAD,m.position_end=l.Curve.Position.NEAR_TOP):e===k?(m.position=l.Curve.Position.NEAR_TOP,m.position_end=l.Curve.Position.NEAR_TOP):(m.position=l.Curve.Position.NEAR_HEAD,m.position_end=l.Curve.Position.NEAR_HEAD):(m.position=l.Curve.Position.NEAR_HEAD,m.position_end=l.Curve.Position.NEAR_HEAD);b.system!==u&&c.system!==u&&b.system!==c.system?(this.createSingleSlur(b,{},{y_shift_start:m.y_shift_start,y_shift_end:m.y_shift_end,invert:1===
k&&1===q||-1===k&&-1===q,position:m.position,position_end:m.position}),m.position=m.position_end,m.invert=1===k&&1===r||-1===k&&-1===r,this.createSingleSlur({},c,m)):this.createSingleSlur(b,c,m)}return this},createSingleSlur:function(a,b,c){this.allVexObjects.push(new l.Curve(a.vexNote,b.vexNote,c))},bezierStringToCps:function(a){var b=[],c,d;for(c=/(\-?[\d|\.]+)\s+(\-?[\d|\.]+)/g;d=c.exec(a);)b.push({x:+d[1],y:+d[2]});b[1]||(w.info("Incomplete attribute","Expected four control points in slur/@bezier, but only found two. Providing cps 3 & 4 on basis on cps 1 & 2."),
b[1]={x:-b[0].x,y:b[0].y});return b}});z=function(a,b){this.init(a,b)};z.prototype={BOTTOM:l.Annotation.VerticalJustify.BOTTOM,init:function(a,b){this.allVexObjects=[];this.allModels=[];this.systemInfo=a;this.font=b},addModel:function(a){this.allModels.push(a)},getModels:function(){return this.allModels},createInfos:function(a,b){var c,d,f,e,g;c=0;for(d=a.length;c<d;c++){f=a[c];e=r.attsToObj(f);if(g=e.startid)g=g.substring(1);else if(g=e.tstamp){var h=f,k=b,l=void 0,m=void 0,l=h.getAttribute("staff")||
"1",m=h.getAttribute("layer")||"1",p=k.querySelector('staff[n="'+l+'"]');if(!p)throw new x('Could not find staff @n="'+l+'" in '+r.serializeElement(k)+" while processing "+r.serializeElement(h));var q=p.querySelector('layer[n="'+m+'"]');if(!q&&((p=p.getElementsByTagName("layer")[0])&&!p.hasAttribute("n")&&(q=p),!q))throw new x('Could not find layer @n="'+m+'" in '+r.serializeElement(k)+" while processing "+r.serializeElement(h));h=this.systemInfo.getStaveInfo(l);if(!h)throw new x("Cannot determine staff definition.");
h=h.getTimeSpec();if(!h.count||!h.unit)throw new x("Cannot determine meter; missing or incorrect @meter.count or @meter.unit.");g=MeiLib.tstamp2id(g,q,h)}else{w.warn("@startid or @tstamp expected",r.serializeElement(f)+" could not be processed because neither @startid nor @tstamp are specified.");break}this.allModels.push({element:f,atts:e,startid:g})}},createVexFromInfos:function(a){var b,c,d;for(b=this.allModels.length;b--;)c=this.allModels[b],(d=a[c.startid])?this.addToNote(c,d):c.startid?w.warn("Unknown reference",
r.serializeElement(c.element)+' could not be processed because the reference "'+c.startid+'" could not be resolved.'):w.warn("Unknown reference",r.serializeElement(c.element)+" could not be processed because it could not be assigned to an element.")},addToNote:function(){throw new x("You have to provide an addToNote() method when inheriting MEI2VF.EventPointerCollection.");}};var X=function(a){this.init(a)};Vex.Inherit(X,z,{init:function(a,b){X.superclass.init.call(this,a,b)},createInfos:function(a,
b){var c,d,f,e,g;c=0;for(d=a.length;c<d;c++){f=a[c];e=r.attsToObj(f);var h;e.plist&&(h=r.pListToArray(e.plist));if(h&&h[0])g=h[0].substring(1);else if(g=e.tstamp){var k=f,l=b,m=void 0,p=void 0,m=k.getAttribute("staff")||"1",p=k.getAttribute("layer")||"1",q=l.querySelector('staff[n="'+m+'"]');if(!q)throw new x('Could not find staff @n="'+m+'" in '+r.serializeElement(l)+" while processing "+r.serializeElement(k));var t=q.querySelector('layer[n="'+p+'"]');if(!t&&((q=q.getElementsByTagName("layer")[0])&&
!q.hasAttribute("n")&&(t=q),!t))throw new x('Could not find layer @n="'+p+'" in '+r.serializeElement(l)+" while processing "+r.serializeElement(k));k=this.systemInfo.getStaveInfo(m);if(!k)throw new x("Cannot determine staff definition.");k=k.getTimeSpec();if(!k.count||!k.unit)throw new x("Cannot determine meter; missing or incorrect @meter.count or @meter.unit.");g=MeiLib.tstamp2id(g,t,k)}else{w.warn("@startid or @tstamp expected",r.serializeElement(f)+" could not be processed because neither @startid nor @tstamp are specified.");
break}this.allModels.push({element:f,atts:e,startid:g})}},addToNote:function(a,b){b.vexNote.addStroke(0,new l.Stroke(0))}});var Y=function(a,b){this.init(a,b)};Vex.Inherit(Y,z,{init:function(a,b){Y.superclass.init.call(this,a,b)},addToNote:function(a,b){var c,d,f,e;c=this.font.family;d=this.font.size;f="";if(e=a.element.getElementsByTagName("rend")[0])e=r.attsToObj(e),e.fontfamily&&(c=e.fontfamily),e.fontweight&&(f+=e.fontweight+" "),e.fontstyle&&(f+=e.fontstyle),e.fontsize&&(d=1.5*+e.fontsize);c=
(new l.Annotation(r.getNormalizedText(a.element).trim())).setFont(c,d,f).setMeiElement(a.element);c.setWidth(0);c.setJustification(1);"below"===a.atts.place?b.vexNote.addAnnotation(0,c.setVerticalJustification(this.BOTTOM)):b.vexNote.addAnnotation(0,c)}});var Z=function(a,b){this.init(a,b)};Vex.Inherit(Z,z,{init:function(a,b){Z.superclass.init.call(this,a,b)},addToNote:function(a,b){var c=(new l.Annotation(r.getText(a.element).trim())).setFont(this.font.family,this.font.size,this.font.weight).setMeiElement(a.element);
c.setWidth(0);"above"===a.atts.place?b.vexNote.addAnnotation(0,c):b.vexNote.addAnnotation(0,c.setVerticalJustification(this.BOTTOM))}});var aa=function(a,b){this.init(a,b)};Vex.Inherit(aa,z,{init:function(a,b){aa.superclass.init.call(this,a,b)},addToNote:function(a,b){t.addFermata(b.vexNote,a.element,a.atts.place)}});var ba=function(a,b){this.init(a,b)};Vex.Inherit(ba,z,{init:function(a,b){ba.superclass.init.call(this,a,b)},addToNote:function(a,b){var c=a.atts,d={trill:"tr",mordent:"mordent",turn:"turn"}[a.element.localName],
d=new l.Ornament(d+("mordent"===d?"inv"===c.form?"":"_inverted":"inv"===c.form?"_inverted":""));d.setMeiElement(a.element);c.accidupper&&d.setUpperAccidental(L[c.accidupper]);c.accidlower&&d.setLowerAccidental(L[c.accidlower]);b.vexNote.addModifier(0,d)}});z=function(){};z.prototype={init:function(){this.spanElements=[];this.vexObjects=[]},addSpanElements:function(a){this.spanElements.push(a)},addVexObject:function(a){this.vexObjects.push(a)},resolveSpans:function(a,b,c){var d,f,e,g,h,k;d=0;for(f=
a.length;d<f;d++)if(e=a[d],g=e.getAttribute("plist"),g=r.pListToArray(g),h=e.getAttribute("startid"),k=e.getAttribute("endid"),null!==h||null!==k){g[0]!==h&&g.unshift(h);g[g.length-1]!==k&&g.push(k);var n=[],m;g=g.map(function(a,b){var d=c[a.substring(1)];if(!d)throw new x('Reference "'+a+'" given in '+r.serializeElement(e)+" not found.");var f=d.vexNote.voice;0===b&&(m=D(d.meiNote).closest("measure").get(0));var g=n.indexOf(f);-1===g&&(m&&D(d.meiNote).closest("measure").get(0)!==m||(g=n.push(f)-
1));return{obj:d,voiceIndex:g,vexNote:d.vexNote}});var p;h=function(a,b){var c=new l.GhostNote(a.getDuration());c.setStave(b);return c};k=g.map(function(a){return a.vexNote});var q,t;if(1<n.length)for(var B=0,v=n.length;B<v;B++){q=[];t=[];for(var u=0,sa=g.length;u<sa;u++)g[u].voiceIndex===B?(q[u]=g[u].vexNote,t.push(n[B].tickables.indexOf(g[u].vexNote))):-1!==g[u].voiceIndex&&(p=h(g[u].vexNote,n[B].tickables[0].stave),q[u]=p);p=n[B].tickables;0!==B&&"function"===typeof b&&b(e,q);n[B].tickables=p.slice(0,
t[0]).concat(q).concat(p.slice(t[t.length-1]+1))}this.createVexObject(k,n,e)}else w.warn("Missing attributes","Could not process "+r.serializeElement(e)+", because @startid or @endid is missing.")},createVexObject:function(){throw new x("createVexObject() method not implemented.");},postFormat:function(){var a,b,c=this.vexObjects;a=0;for(b=c.length;a<b;a++)c[a].postFormat()},setContext:function(a){this.ctx=a;return this},draw:function(){var a,b,c=this.vexObjects,d=this.ctx;a=0;for(b=c.length;a<b;a++)c[a].setContext(d).draw()}};
var ca=function(){this.init()};Vex.Inherit(ca,z,{init:function(){ca.superclass.init.call(this)},resolveSpanElements:function(a){this.resolveSpans(this.spanElements,null,a)},createVexObject:function(a){this.vexObjects.push(new l.Beam(a,!1))}});var da=function(){this.init()};Vex.Inherit(da,z,{init:function(){da.superclass.init.call(this)},resolveSpanElements:function(a){this.resolveSpans(this.spanElements,function(a,c){new l.Tuplet(c,{num_notes:parseInt(a.getAttribute("num"),10)||3,beats_occupied:parseInt(a.getAttribute("numbase"),
10)||2})},a)},createVexObject:function(a,b,c){var d,f;a=new l.Tuplet(a,{num_notes:parseInt(c.getAttribute("num"),10)||3,beats_occupied:parseInt(c.getAttribute("numbase"),10)||2});"ratio"===c.getAttribute("num.format")&&a.setRatioed(!0);a.setBracketed("true"===c.getAttribute("bracket.visible"));(c=c.getAttribute("bracket.place"))&&a.setTupletLocation("above"===c?1:-1);this.vexObjects.push(a);d=0;for(f=b.length;d<f;d++)a=b[d],c=a.tickables,a.ticksUsed=new Vex.Flow.Fraction(0,1),a.tickables=[],a.addTickables(c)}});
...
c);return this}};var y=function(a,b,c){this.init(a);this.setFont(c.family,c.size,c.weight);this.setMeiElement(b);this.setLineSpacing(c.spacing)};y.CATEGORY="annotations";y.DEFAULT_FONT_SIZE=10;y.Justify={LEFT:1,CENTER:2,RIGHT:3,CENTER_STEM:4};y.VerticalJustify={TOP:1,BOTTOM:3};y.format=function(a,b){if(!a||0===a.length)return!1;for(var c=b.text_line,d,f=0;f<a.length;++f)d=a[f],d.setTextLine(c),d=0<d.getWidth()?d.getWidth():0,c++;b.left_shift+=d/2;b.right_shift+=d/2;return!0};var ia=l.Modifier;Vex.Inherit(y,
ia,{init:function(a){y.superclass.init.call(this);this.index=this.note=null;this.text_line=0;this.text=a;this.justification=y.Justify.CENTER;this.vert_justification=y.VerticalJustify.BOTTOM;this.font={family:"Arial",size:y.DEFAULT_FONT_SIZE,weight:""};this.line_spacing=1.1;this.setWidth(l.textWidth(a))},setMeiElement:function(a){this.meiElement=a;return this},getMeiElement:function(){return this.meiElement},setLineSpacing:function(a){this.line_spacing=a;return this},setTextLine:function(a){this.text_line=
a;return this},setFont:function(a,b,c){this.font={family:a,size:b,weight:c};return this},setVerticalJustification:function(a){this.vert_justification=a;return this},getJustification:function(){return this.justification},setJustification:function(a){this.justification=a;return this},preProcess:function(){var a,b,c,d=this.note.hasStem(),f=this.note.getStave();d&&(b=this.note.getStem().getExtents(),c=f.getSpacingBetweenLines());var e=this.font.size/y.DEFAULT_FONT_SIZE*this.line_spacing;this.vert_justification==
y.VerticalJustify.BOTTOM?(a=f.getYForBottomText(this.text_line),d&&(b=1===this.note.getStemDirection()?b.baseY+10:b.topY+5,a=Math.max(a,b+(c*(this.text_line+1)*e+c*this.text_line)))):this.vert_justification==y.VerticalJustify.TOP&&(a=Math.min(f.getYForTopText(this.text_line),this.note.getYs()[0]-10),d&&(a=Math.min(a,b.topY-5-c*this.text_line)));return this.y=a},setY:function(a){this.y=a},draw:function(){if(!this.context)throw new Vex.RERR("NoContext","Can't draw text annotation without a context.");
if(!this.note)throw new Vex.RERR("NoNoteForSyllable","Can't draw text annotation without an attached note.");var a=this.note.getModifierStartXY(ia.Position.ABOVE,this.index);this.context.save();this.context.setFont(this.font.family,this.font.size,this.font.weight);var b=this.context.measureText(this.text).width,c=this.context.measureText("m").width,d;this.x=a=this.justification==y.Justify.LEFT?a.x:this.justification==y.Justify.RIGHT?a.x-b:this.justification==y.Justify.CENTER?a.x-b/2:this.note.getStemX()-
...
invis:null},init:function(a){var b,c,d,f,e,g,h,k,n,m=a.models,p=a.staves;a.barlineInfo&&(k=a.barlineInfo.leftBarline,n=a.barlineInfo.rightBarline);var q=a.system_n;e=a.labelMode;for(g in m)h=m[g],a=n?this.vexTypesBarlineRight[n]:this.vexTypes[h.symbol],b=p[h.top_stave_n],c=p[h.bottom_stave_n],"number"===typeof a&&b&&c&&(d=new l.StaveConnector(b,c),d.setType(a),this.allVexConnectors.push(d),"full"===e?f=0===q?h.label:h.labelAbbr:"abbr"===e&&(f=h.labelAbbr),f&&d.setText(f)),k&&(a=this.vexTypesBarlineLeft[k],
...
h=this.keySpec.meiElement,k=this.timeSpec.meiElement;if(a){var l="clef"===g.localName?"":"clef.",m="clef"===a.localName?"":"clef.";e(g,a,l,m,"shape")&&e(g,a,l,m,"line")||(d.clef=!0)}!b||f(h,b,"key.pname")&&f(h,b,"key.accid")&&f(h,b,"key.mode")||(d.keysig=!0);!c||f(k,c,"meter.count")&&f(k,c,"meter.unit")&&f(k,c,"meter.sym")||(d.timesig=!0);this.renderWith=d},updateLabels:function(a){var b;b=a.getAttribute("label");"string"===typeof b&&(this.label=b);a=a.getAttribute("label.abbr");"string"===typeof a&&
...
null)},alignRestsToNotes:function(a,b,c){for(var d=function(a,b,c,d){var e=b;for(c++;c<a.length;){if(!a[c].isRest()&&!a[c].shouldIgnoreTicks()){e=a[c].getLineForRest();break}c++}d&&b!=e&&(a=Vex.Max(b,e),b=Vex.Min(b,e),e=Vex.MidLine(a,b));return e},f=0;f<a.length;++f)if(a[f]instanceof Vex.Flow.StaveNote&&a[f].isRest()&&!a[f].manualPosition){var e=a[f];if(!e.tuplet||c){var g=e.getGlyph().position.toUpperCase();if("R/4"==g||"B/4"==g)if(b||null!=e.beam)if(g=e.getKeyProps()[0],0===f)g.line=d(a,g.line,
f,!1),e.setKeyLine(0,g.line);else if(0<f&&f<a.length){var h;a[f-1].isRest()?(h=a[f-1].getKeyProps()[0].line,g.line=h):(h=a[f-1].getLineForRest(),g.line=d(a,h,f,!0));e.setKeyLine(0,g.line)}}}},getYBounds:function(){var a=this.vexVoicesStaffWise,b={},c;for(c in a){b[c]=[];for(var d=0,f=a[c].length;d<f;d++)b[c].push(a[c][d].getBoundingBox())}return b},draw:function(a){var b,c,d=this.all_voices;for(b=0;b<d.length;++b)c=d[b],this.drawVoice.call(c.voice,a)},drawVoice:function(a){for(var b=null,c=0;c<this.tickables.length;++c){var d=
this.tickables[c];if(!d.getStave())throw new Vex.RuntimeError("MissingStave","The voice cannot draw tickables without staves.");d.setStave(d.getStave());0===c&&(b=d.getBoundingBox());if(0<c&&b){var f=d.getBoundingBox();f&&b.mergeWith(f)}d.setContext(a);d.draw()}this.boundingBox=b}};var C=function(a){a&&this.initConfig(a);return this};C.prototype={defaults:{pageWidth:null,pageTopMar:60,pageBottomMar:80,pageLeftMar:20,pageRightMar:20,defaultSpacingInMeasure:180,systemSpacing:90,staveSpacing:60,autoStaveConnectorLine:!0,
labelMode:null,readMeasureWidths:!0,processSb:"sb",processPb:"sb",maxHyphenDistance:75,lyricsFont:{family:"Times",size:15,spacing:1.3},annotFont:{family:"Times",size:15},dynamFont:{family:"Times",size:17.5,weight:"bold italic"},tempoFont:{family:"Times",size:17,weight:"bold"}},initConfig:function(a){this.cfg=r.extend({},this.defaults,a);this.systemInfo=new pa;this.pageInfo=new ma(this.cfg);switch(this.cfg.processSb){case "pb":this.onSb=this.setPendingPageBreak;break;case "sb":this.onSb=this.setPendingSystemBreak;
break;default:this.onSb=this.emptyFn}switch(this.cfg.processPb){case "pb":this.onPb=this.setPendingPageBreak;break;case "sb":this.onPb=this.setPendingSystemBreak;break;default:this.onPb=this.emptyFn}return this},emptyFn:function(){},reset:function(){this.page=new la;this.systemInfo.init(this.cfg);this.unresolvedTStamp2=[];this.allVexMeasureStaves=[];this.beams=new ca;this.tuplets=new da;this.dynamics=new Z(this.systemInfo,this.cfg.dynamFont);this.arpeggios=new X(this.systemInfo);this.directives=new Y(this.systemInfo,
this.cfg.annotFont);this.fermatas=new aa(this.systemInfo);this.ornaments=new ba(this.systemInfo);this.ties=new V(this.systemInfo,this.unresolvedTStamp2);this.slurs=new W(this.systemInfo,this.unresolvedTStamp2);this.hairpins=new U(this.systemInfo,this.unresolvedTStamp2);this.notes_by_id={};this.currentSystem_n=-1;this.pendingSystemBreak=!1;this.pendingSectionBreak=!0;this.voltaInfo=null;return this},process:function(a){"score"===a.localName?this.processScoreChildren(a):this.processScoreChildren(a.querySelector("score"));
this.arpeggios.createVexFromInfos(this.notes_by_id);this.dynamics.createVexFromInfos(this.notes_by_id);this.ornaments.createVexFromInfos(this.notes_by_id);this.directives.createVexFromInfos(this.notes_by_id);this.fermatas.createVexFromInfos(this.notes_by_id);this.ties.createVexFromInfos(this.notes_by_id);this.slurs.createVexFromInfos(this.notes_by_id);this.hairpins.createVexFromInfos(this.notes_by_id);this.tuplets.resolveSpanElements(this.notes_by_id);this.beams.resolveSpanElements(this.notes_by_id);
return this},format:function(a){this.page.formatSystems(this.pageInfo,this.systemInfo,this.cfg,a);this.beams.postFormat()},draw:function(a){this.page.setContext(a).drawSystems();this.beams.setContext(a).draw();this.tuplets.setContext(a).draw();this.ties.setContext(a).draw();this.slurs.setContext(a).draw();this.hairpins.setContext(a).draw();return this},getStaveArea:function(){var a;a=this.systemInfo.getCurrentLowestY();var b=this.getAllVexMeasureStaves(),c,d,f,e,g;c=b.length;for(e=0;c--;)if(b[c]){f=
0;for(d=b[c].length;d--;)(g=b[c][d])&&(f=Math.max(f,g.getNoteStartX()));for(d=b[c].length;d--;)(g=b[c][d])&&(e=Math.max(e,f+g.getWidth()))}return{width:e,height:a}},getAllVexMeasureStaves:function(){return this.allVexMeasureStaves},getSystems:function(){return this.page.getSystems()},getNotes:function(){return this.notes_by_id},createNewSystem:function(){var a;w.debug("Converter.createNewSystem()","{enter}");this.pendingSystemBreak=!1;this.currentSystem_n+=1;a=new na(this.pageInfo,this.systemInfo,
this.currentSystem_n);this.pendingSectionBreak?(this.pendingSectionBreak=!1,this.systemInfo.forceSectionStartInfos()):this.systemInfo.forceStaveStartInfos();this.page.addSystem(a,this.currentSystem_n);return a},processScoreChildren:function(a){var b,c,d;d={voltaInfo:null};if(a)for(c=a.childNodes,a=0,b=c.length;a<b;a++)1===c[a].nodeType&&this.processScoreChild(c[a],d);else throw new x("No score element found in the document.");},processScoreChild:function(a,b){switch(a.localName){case "scoreDef":this.systemInfo.processScoreDef(a);
break;case "staffDef":this.systemInfo.processStaffDef(a,null);break;case "pb":this.onPb(a);break;case "ending":this.processEnding(a,b);break;case "section":this.processSection(a,b);break;default:w.info("Not supported","Element "+r.serializeElement(a)+" is not supported in <score>. Ignoring element.")}},processSection:function(a,b){var c,d,f=a.childNodes;c=0;for(d=f.length;c<d;c+=1)1===f[c].nodeType&&this.processSectionChild(f[c],b)},processEnding:function(a,b){var c,d,f;f=null===b.voltaInfo;var e=
function(a){for(a=a.nextSibling;a&&1!==a.nodeType;)a=a.nextSibling;return a};d=a.firstChild;for(1!==d.nodeType&&(d=e(d));d;)c=e(d),"measure"===d.localName?(null===b.voltaInfo?b.voltaInfo={start:a.getAttribute("n")}:delete b.voltaInfo.start,!c&&f&&(b.voltaInfo.end=!0)):"ending"!==d.localName&&"section"!==d.localName||w.info("Not supported",r.serializeElement(d)+" is not supported as a child of "+r.serializeElement(a)+". Trying to process it anyway."),this.processSectionChild(d,b),d=c;f&&(b.voltaInfo=
null)},processSectionChild:function(a,b){switch(a.localName){case "measure":this.processMeasure(a,b);break;case "scoreDef":this.systemInfo.processScoreDef(a);break;case "staffDef":this.systemInfo.processStaffDef(a,null);break;case "pb":this.onPb(a);break;case "sb":this.onSb(a);break;case "ending":this.processEnding(a,b);break;case "section":this.processSection(a,b);break;default:w.info("Not supported","Element "+r.serializeElement(a)+" is not supported in <section>. Ignoring element.")}},setPendingPageBreak:function(){w.info("setPendingPageBreak() not implemented.")},
setPendingSystemBreak:function(){this.pendingSystemBreak=!0},processMeasure:function(a,b){var c,d,f;this.pendingSectionBreak||this.pendingSystemBreak?(d=this.createNewSystem(),c=!0):(d=this.page.getSystems(),c=d.length-1,d=d[c],c=!1);w.debug("Converter.processMeasure()","{enter}");var e={leftBarline:a.getAttribute("left"),rightBarline:a.getAttribute("right")};b.leftBarlineElement&&!e.leftBarline&&(e.leftBarline=b.leftBarlineElement.getAttribute("right"),e.leftBarlineElement=b.leftBarlineElement,b.leftBarlineElement=
null);"rptstart"===e.rightBarline&&(e.rightBarline=null,b.leftBarlineElement=a);var g=[],h=[],k=[],l=[],m=[],p=[],q=[],r=[],t=[],v=[],x=[],u,y;f=a.childNodes;u=0;for(y=f.length;u<y;u++)switch(f[u].localName){case null:break;case "staff":g.push(f[u]);break;case "dir":k.push(f[u]);break;case "harm":k.push(f[u]);break;case "tie":m.push(f[u]);break;case "slur":l.push(f[u]);break;case "hairpin":p.push(f[u]);break;case "tempo":q.push(f[u]);break;case "dynam":r.push(f[u]);break;case "arpeg":h.push(f[u]);
break;case "fermata":t.push(f[u]);break;case "mordent":case "turn":case "trill":x.push(f[u]);break;case "reh":v.push(f[u]);break;case "beamSpan":this.beams.addSpanElements(f[u]);break;case "tupletSpan":this.tuplets.addSpanElements(f[u]);break;default:w.info("Not supported","<"+f[u].localName+"> is not supported as child of <measure>.")}f=this.initializeStavesInMeasure(d,g,e,c,b);var z=this.allVexMeasureStaves.push(f)-1,C=new qa,D=new ga(this.notes_by_id,this.currentSystem_n);u=0;for(y=g.length;u<
y;u++)this.processStaveEvents(f,g[u],z,C,D);0!==D.clefCheckQueue.length&&this.processClefCheckQueue(D);this.dynamics.createInfos(r,a);this.arpeggios.createInfos(h,a);this.directives.createInfos(k,a);this.fermatas.createInfos(t,a);this.ornaments.createInfos(x,a);this.ties.createInfos(m,a,z,this.systemInfo);this.slurs.createInfos(l,a,z,this.systemInfo);this.hairpins.createInfos(p,a,z,this.systemInfo);c=new ka({system:d,element:a,staves:f,voices:C,startConnectorCfg:c?{labelMode:this.cfg.labelMode,models:this.systemInfo.startConnectorInfos,
staves:f,system_n:this.currentSystem_n}:null,inlineConnectorCfg:{models:this.systemInfo.inlineConnectorInfos,staves:f,barlineInfo:e},tempoElements:q,rehElements:v,tempoFont:this.cfg.tempoFont,readMeasureWidths:this.cfg.readMeasureWidths});d.addMeasure(c)},processClefCheckQueue:function(a){var b,c,d,f=a.clefCheckQueue;b=0;for(c=f.length;b<c;b++)d=f[b],d.clef!==d.stave.clef&&(d.clef=d.stave.clef,d.keyProps=[],d.calculateKeyProps(),d.buildNoteHeads(),d.buildStem());a.emptyClefCheckQueue()},initializeStavesInMeasure:function(a,
b,c,d,f){var e,g,h,k,l=!0,m={},p=0,q={},t=0,u,v;k=[];d||(u=a.getLastMeasure().getStaves());d=0;for(e=b.length;d<e;d++){h=parseInt(b[d].getAttribute("n"),10);if(isNaN(h))throw new x(r.serializeElement(b[d])+" must have an @n attribute of type integer.");g=new ja({system:a,y:a.getStaveYs()[h],barlineInfo:c});k[h]=g;l&&f.voltaInfo&&(g.addVoltaFromInfo(f.voltaInfo),l=!1);if(u&&u[h])v=this.systemInfo.getStaveInfo(h),p=v.getClef(),v.showClefCheck()&&u[h].addEndClefFromInfo(p),g.clef=p.type,p=m[h]=0;else{v=
this.systemInfo.getStaveInfo(h);if(!v)throw new x(r.serializeElement(b[d])+' refers to stave "'+h+'", but no corresponding stave definition could be found in the document.');v.showClefCheck()&&g.addClefFromInfo(v.getClef());m[h]=g.getModifierXShift();p=Math.max(p,m[h])}}e=k.length;for(d=0;d<e;d++)if(g=k[d])a=m[d]!==p?p-m[d]+10:null,v=this.systemInfo.getStaveInfo(d),v.showKeysigCheck()&&g.addKeySpecFromInfo(v.getKeySpec(),a),q[d]=g.getModifierXShift(),t=Math.max(t,q[d]);for(d=0;d<e;d++)if(g=k[d])a=
q[d]!==t?t-q[d]+15:null,v=this.systemInfo.getStaveInfo(d),v.showTimesigCheck()&&g.addTimeSpecFromInfo(v.getTimeSpec(),a);return k},processStaveEvents:function(a,b,c,d,f){var e,g,h,k,n,m;e=parseInt(b.getAttribute("n"),10)||1;a=a[e];f.startNewStave(a,e);m=this.systemInfo.getStaveInfo(e);var p=m.getTimeSpec();g=b.getElementsByTagName("layer");h=0;for(k=g.length;h<k;h++)f.setLayerDir(1<k?0===h?l.StaveNote.STEM_UP:h===k-1?l.StaveNote.STEM_DOWN:null:null),this.resolveUnresolvedTimestamps(g[h],e,c,p),m.checkInitialClef(),
n=this.processNoteLikeChildren(f,g[h],m),d.addVoice(this.createVexVoice(n,p),e);b=b.getElementsByTagName("anchoredText");h=0;for(k=b.length;h<k;h++)this.processAnchoredText(f,b[h]);f.clefChangeInfo&&(a.addEndClefFromInfo(f.clefChangeInfo),f.setClefChangeInfo(null));m.finalizeClefInfo()},createVexVoice:function(a,b){var c;if(!Array.isArray(a))throw new x("me.createVexVoice() voice_contents argument must be an array.");c=new l.Voice({num_beats:b.count,beat_value:b.unit,resolution:l.RESOLUTION});c.setStrict(!1);
c.addTickables(a);return c},resolveUnresolvedTimestamps:function(a,b,c,d){var f,e;b=c+":"+(b||1)+":"+(parseInt(a.getAttribute("n"),10)||"1");if(e=this.unresolvedTStamp2[b]){c=0;for(f=e.length;c<f;c++)e[c].setContext({layer:a,meter:d}),e[c]=null;this.unresolvedTStamp2[b]=null}},processNoteLikeChildren:function(a,b,c){var d=[],f,e,g=b.childNodes;b=0;for(f=g.length;b<f;b++)1===g[b].nodeType&&(e=this.processNoteLikeElement(a,g[b],c))&&(Array.isArray(e)?d=d.concat(e):d.push(e));return d},processNoteLikeElement:function(a,
b,c){switch(b.localName){case "rest":return this.processRest(a,b,c);case "mRest":return this.processMRest(a,b,c);case "space":return this.processSpace(a,b);case "note":return this.processNote(a,b,c);case "beam":return this.processBeam(a,b,c);case "tuplet":return this.processTuplet(a,b,c);case "chord":return this.processChord(a,b,c);case "clef":return this.processClef(a,b,c);case "bTrem":return this.processBTrem(a,b,c);default:w.info("Not supported",'Element "'+b.localName+'" is not supported. Ignoring element.')}},
processAnchoredText:function(a,b){},processNote:function(a,b,c){var d,f,e,g,h,k,n,m,p,q;g=r.attsToObj(b);f=g.tie;e=g.slur;d=MeiLib.XMLID(b);g.staff=parseInt(g.staff);try{if(m=t.getVexPitch(b),g.staff&&((q=this.allVexMeasureStaves[this.allVexMeasureStaves.length-1][g.staff])?(p=q,n=this.systemInfo.getClef(g.staff)):w.warn("Staff not found",'No stave could be found which corresponds to @staff="'+g.staff+'" specified in '+r.serializeElement(b)+'". Adding note to current stave.')),n||(n=c.getClef()),
p||(p=a.getStave()),h={vexPitch:m,clef:n,element:b,atts:g,stave:p,layerDir:a.getLayerDir()},k=g.grace?new O(h):new N(h),q&&a.addToClefCheckQueue(k),k.hasMeiStemDir&&a.isInBeam()&&a.setStemDirInBeam(!0),this.processSyllables(k,b,a.stave_n),f&&this.processAttrTie(f,d,m,g.staff||a.stave_n),e&&this.processSlurAttribute(e,d),a.addEvent(d,{meiNote:b,vexNote:k}),a.clefChangeInfo&&(t.addClefModifier(k,a.clefChangeInfo),a.setClefChangeInfo(null)),g.grace)a.graceNoteQueue.push(k);else return 0<a.graceNoteQueue.length&&
(k.addModifier(0,(new l.GraceNoteGroup(a.graceNoteQueue,!1)).beamNotes()),a.graceNoteQueue=[]),k}catch(u){throw new x("An error occurred processing "+r.serializeElement(b)+': "'+u.toString());}},processChord:function(a,b,c){var d,f,e,g,h,k,n,m,p,q,u;d=b.getElementsByTagName("note");h=r.attsToObj(b);m=h.slur;f=MeiLib.XMLID(b);h.staff=parseInt(h.staff);try{h.staff&&((u=this.allVexMeasureStaves[this.allVexMeasureStaves.length-1][h.staff])?(q=u,p=this.systemInfo.getClef(h.staff)):w.warn("Staff not found",
'No stave could be found which corresponds to @staff="'+h.staff+'" specified in '+r.serializeElement(b)+'". Adding note to current stave.'));p||(p=c.getClef());q||(q=a.getStave());g={noteElements:d,clef:p,stave:q,element:b,atts:h,layerDir:a.getLayerDir()};e=h.grace?new Q(g):new P(g);u&&a.addToClefCheckQueue(e);e.hasMeiStemDir&&a.isInBeam()&&a.setStemDirInBeam(!0);c=[];k=0;for(n=d.length;k<n;k++)this.processNoteInChord(a,k,d[k],b,e),c.push(k);h.tie&&this.processAttrTie(h.tie,f,null,h.staff||a.stave_n);
m&&this.processSlurAttribute(m,f);a.addEvent(f,{meiNote:b,vexNote:e,index:c});a.clefChangeInfo&&(t.addClefModifier(e,a.clefChangeInfo),a.setClefChangeInfo(null));if(h.grace)a.graceNoteQueue.push(e);else return 0<a.graceNoteQueue.length&&(e.addModifier(0,(new l.GraceNoteGroup(a.graceNoteQueue,!1)).beamNotes()),a.graceNoteQueue=[]),e}catch(y){a=r.serializeElement(b);k=0;for(n=d.length;k<n;k++)a+="\n    "+r.serializeElement(d[k]);throw new x("A problem occurred processing \n"+a+"\n</chord>\n: "+y.toString());
}},processNoteInChord:function(a,b,c,d,f){var e,g;e=r.attsToObj(c);var h=t.getVexPitch(c);g=MeiLib.XMLID(c);e.tie&&this.processAttrTie(e.tie,g,h,parseInt(e.staff)||a.stave_n);e.slur&&this.processSlurAttribute(e.slur,g);a.addEvent(g,{meiNote:d,vexNote:f,index:[b]});g=c.childNodes;a=0;for(d=g.length;a<d;a++)switch(g[a].localName){case "accid":e.accid=g[a].getAttribute("accid");break;case "artic":t.addArticulation(f,g[a])}e.accid&&t.processAttrAccid(e.accid,f,b);e.fermata&&t.addFermata(f,c,e.fermata,
b)},processRest:function(a,b,c){var d,f;try{return d=new R({element:b,stave:a.getStave(),clef:b.hasAttribute("ploc")&&b.hasAttribute("oloc")?c.getClef():null}),f=MeiLib.XMLID(b),a.clefChangeInfo&&(t.addClefModifier(d,a.clefChangeInfo),a.setClefChangeInfo(null)),0<a.graceNoteQueue.length&&(d.addModifier(0,(new l.GraceNoteGroup(a.graceNoteQueue,!1)).beamNotes()),a.graceNoteQueue=[]),a.addEvent(f,{meiNote:b,vexNote:d}),d}catch(e){throw new x("An error occurred processing "+r.serializeElement(b)+': "'+
e.toString());}},processMRest:function(a,b,c){var d,f;try{var e={meter:c.getTimeSpec(),element:b,stave:a.getStave(),clef:b.hasAttribute("ploc")&&b.hasAttribute("oloc")?c.getClef():null};d=new S(e);f=MeiLib.XMLID(b);0<a.graceNoteQueue.length&&(d.addModifier(0,(new l.GraceNoteGroup(a.graceNoteQueue,!1)).beamNotes()),a.graceNoteQueue=[]);a.addEvent(f,{meiNote:b,vexNote:d});return d}catch(g){throw new x("An error occurred processing "+r.serializeElement(b)+': "'+g.toString());}},processSpace:function(a,
b){var c=null;if(b.hasAttribute("dur"))try{c=new I({element:b,stave:a.getStave()}),a.isInBeam()&&a.setSpaceInBeam(!0),0<a.graceNoteQueue.length&&(c.addModifier(0,(new l.GraceNoteGroup(a.graceNoteQueue,!1)).beamNotes()),a.graceNoteQueue=[])}catch(d){throw new x("An error occurred processing "+r.serializeElement(b)+': "'+d.toString());}else w.info("@dur expected","No duration attribute in "+r.serializeElement(b)+'". Ignoring element.');return c},processClef:function(a,b,c){a.setClefChangeInfo(c.clefChangeInMeasure(b))},
processBTrem:function(a,b,c){w.info("Not implemented","Element <bTrem> not implemented. Processing child nodes.");return this.processNoteLikeChildren(a,b,c)},processBeam:function(a,b,c){var d,f,e;a.enterBeam();c=this.processNoteLikeChildren(a,b,c);if(!0===a.getSpaceInBeam())if(e=a.shiftBeamInfoToResolve(),e!==u){var g=[];f=c.length;f!==e.vexNotes.length&&w.warn("Beam content mismatch",r.serializeElement(b)+" and "+r.serializeElement(e.element)+" could not be combined, because their content does not match.");
for(d=0;d<f;d++)c[d]instanceof I?g.push(e.vexNotes[d]):g.push(c[d]);d=g.filter(function(a){return a&&!0===a.beamable})}else a.addBeamInfoToResolve(b,c);else d=c.filter(function(a){return!0===a.beamable});if(d&&1<d.length)try{this.beams.addVexObject(new l.Beam(d,!a.getLayerDir()&&!1===a.getStemDirInBeam()))}catch(h){w.error("VexFlow Error","An error occurred processing "+r.serializeElement(b)+': "'+h.toString()+'". Ignoring beam.')}a.exitBeam();return c},processTuplet:function(a,b,c){a=this.processNoteLikeChildren(a,
b,c);if(0===a.length)w.warn("Missing content","Not content found in "+r.serializeElement(b)+'". Ignoring tuplet.');else return c=new l.Tuplet(a,{num_notes:parseInt(b.getAttribute("num"),10)||3,beats_occupied:parseInt(b.getAttribute("numbase"),10)||2}),"ratio"===b.getAttribute("num.format")&&c.setRatioed(!0),c.setBracketed("true"===b.getAttribute("bracket.visible")),(b=b.getAttribute("bracket.place"))&&c.setTupletLocation("above"===b?1:-1),this.tuplets.addVexObject(c),a},processAttrTie:function(a,
b,c,d){var f,e;f=0;for(e=a.length;f<e;++f)"t"!==a[f]&&"m"!==a[f]||this.ties.terminateTie(b,{vexPitch:c,stave_n:d}),"i"!==a[f]&&"m"!==a[f]||this.ties.startTie(b,{vexPitch:c,stave_n:d})},processSlurAttribute:function(a,b){var c,d,f,e;if(a)for(c=this.parseSlurAttribute(a),f=0,e=c.length;f<e;f++)d=c[f],"t"===d.letter&&this.slurs.terminateSlur(b,{nesting_level:d.nesting_level}),"i"===d.letter&&this.slurs.startSlur(b,{nesting_level:d.nesting_level})},parseSlurAttribute:function(a){var b=[],c,d,f,e;a=a.split(" ");
d=0;for(f=a.length;d<f;d+=1)if(c=a[d],1===c.length)b.push({letter:c,nesting_level:0});else if(2===c.length){e=+c[1];if(!e)throw new x("badly formed slur attribute");b.push({letter:c[0],nesting_level:e})}else throw new x("badly formed slur attribute");return b},processSyllables:function(a,b,c){var d,f,e;f=b.getElementsByTagName("syl");b.hasAttribute("syl")&&(e=new y(b.getAttribute("syl").replace(/\s+/g," "),b,this.cfg.lyricsFont),a.addAnnotation(0,e),this.page.getSystems()[this.currentSystem_n].verses.addSyllable(e,
b,c));b=0;for(d=f.length;b<d;b++)e=new y(r.getNormalizedText(f[b]),f[b],this.cfg.lyricsFont),a.addAnnotation(0,e),this.page.getSystems()[this.currentSystem_n].verses.addSyllable(e,f[b],c)}};window.MeiLib=MeiLib;window.MEI2VF={setLogging:function(a){w.setLevel.call(w,a)},setLoggerAppender:function(a){w.setAppender.call(w,a)},Converter:{initConfig:function(a){C.prototype.initConfig(a)},reset:function(){C.prototype.reset()},process:function(a){C.prototype.process(a)},format:function(a){C.prototype.format(a)},
draw:function(a){C.prototype.draw(a)},getAllVexMeasureStaffs:function(){return C.prototype.getAllVexMeasureStaves()},getStaffArea:function(){return C.prototype.getStaveArea()}},rendered_measures:null,render_notation:function(a,b,c,d,f,e,g){e=e||{};b=(new l.Renderer(b,f||l.Renderer.Backends.CANVAS)).getContext();e.pageWidth=c;this.Converter.initConfig(e);this.Converter.reset();this.Converter.process(a[0]||a);this.Converter.format(b);this.calculatedHeight=d?null:C.prototype.pageInfo.getCalculatedHeight();
C.prototype.pageInfo.hasCalculatedWidth()?this.calculatedWidth=C.prototype.pageInfo.getCalculatedWidth():this.calculatedWidth=null;+f===l.Renderer.Backends.RAPHAEL&&b.paper.setSize(this.calculatedWidth||c,this.calculatedHeight||d);g&&g(this.calculatedHeight,this.calculatedWidth);this.Converter.draw(b);this.rendered_measures=this.Converter.getAllVexMeasureStaffs()}}})(jQuery,Vex.Flow);
